  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    if (errors.length === 0) {
      ...
      try {
        ...
        // Add lead to store and mark as submitted
        const lead = {
          name: formData.name,
          email: formData.email,
          industry: formData.industry,
      ...
      <div className="bg-gradient-card p-8 rounded-2xl shadow-card border border-border backdrop-blur-sm animate-slide-up">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <Button
            ...
            className="w-full h-12 bg-gradient-primary text-primary-foreground font-semibold rounded-lg shadow-glow hover:shadow-[0_0_60px_hsl(210_100%_60%_0.3)] transition-smooth transform hover:scale-[1.02] disabled:opacity-50 disabled:cursor-not-allowed"
          >
            {isSubmitting ? (
              <>
